...
class NeoRing {
   private:
    ...
    float frameInterval = 1000.0f / fps;  // ms per frame

    AnimationEngine engine;  // declaration moved after frameInterval for clarity

   public:
    // Constructor: takes number of LEDs and pin
    NeoRing(size_t ledCount_, uint8_t pin)
        : ledCount(ledCount_), engine(std::make_unique<NeoPixelDriver>(ledCount_, pin)) {}
    ...
    void solid(uint32_t color) {
        engine.addAnimation(std::make_unique<SolidColor>(color, ledCount));
    }
    ...
    void breathe(uint32_t color, float speed = 1.0f) {
        engine.addAnimation(std::make_unique<Breathe>(color, ledCount, speed));
    }

    void wave(float speed = 1.0f) { engine.addAnimation(std::make_unique<Wave>(ledCount, speed)); }

    void blank() { engine.addAnimation(std::make_unique<SolidColor>(Color::Black, ledCount)); }
