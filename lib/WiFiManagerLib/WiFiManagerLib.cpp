...
WiFiManagerLib::WiFiManagerLib(Display& display, Speaker& speaker)
    : display(display), speaker(speaker), wifiConnected(false) {}

bool WiFiManagerLib::begin(const char* apName, int apTimeout, int connectTimeout) {
...
void WiFiManagerLib::setupWiFiManager(const char* apName, int apTimeout, int connectTimeout) {
    ...
    wifiManager.setCustomHeadElement(
        ...
        "select{width:100%;padding:12px;border:2px solid "
        "#ddd;border-radius:6px;font-size:16px;box-sizing:border-box;"
        "transition:border-color 0.3s;color:white;background-color:rgba(255,255,255,0.1);}"
        "select:focus{border-color:#667eea;outline:none;}"
        "select option{background-color:#667eea;color:white;}"
...
bool WiFiManagerLib::isConnected() const { return wifiConnected && WiFi.status() == WL_CONNECTED; }

void WiFiManagerLib::update() {
    if (isConnected()) {
        ArduinoOTA.handle();  // Handle OTA updates only if WiFi is connected
    }
}

void WiFiManagerLib::resetWiFi() {
